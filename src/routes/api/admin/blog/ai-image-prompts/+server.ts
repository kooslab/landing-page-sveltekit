...
export const POST: RequestHandler = async ({ request, locals }) => {
	...
	try {
		const result = await runImagePrompts({ apiKey, model, draft, signal: request.signal });
		return json({ prompts: result.prompts });
	} catch (err) {
		...
		return json({ error: 'Image prompt generation failed' }, { status: 500 });
	}
};
