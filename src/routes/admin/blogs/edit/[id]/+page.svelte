<script lang="ts">
	...
	let aiModel = $state<'claude-haiku-4-5' | 'claude-sonnet-4-6' | 'claude-opus-4-7'>(
		'claude-sonnet-4-6'
	);
...
<div class="max-w-4xl flex-1 px-8 py-8">
	<div class="mb-8 flex items-center justify-between">
		...
		<div class="flex gap-2">
			<Button type="button" disabled={loading || !hasChanges} onclick={() => handleSubmit()}>
				{loading ? 'Saving...' : hasChanges ? 'Update Article' : 'No changes'}
			</Button>
			{#if !published}
				<Button
					type="button"
	...
	<form onsubmit={handleSubmit} class="space-y-6">
		...
		<div class="space-y-2">
			...
			<Editor bind:content {aiModel} {highlightRange} onOpenAIPanel={() => (aiPanelOpen = true)} />
...
<AIReviewPanel
	...
	bind:content
	bind:model={aiModel}
	onHighlight={(range) => (highlightRange = range)}
/>
